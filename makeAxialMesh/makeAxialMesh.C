...
void changeCoordinates(polyMesh &mesh,plane cutPlane,linie &axisLine,scalar offset,bool revolve) {
  ...
  const scalar factor=std::sin(angle/180.*Foam::constant::mathematical::pi);
  const scalar factorRadius=std::cos(angle/180.*Foam::constant::mathematical::pi);
  vector axisDir = axisLine.vec()/axisLine.mag();      
...
#   include "createTime.H"
    ...
    changeCoordinates(mesh,approx,theAxis,offset,revolve);
